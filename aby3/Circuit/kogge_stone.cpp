...
KoggeStoneLibrary::BetaCircuit* KoggeStoneLibrary::int_int_add(u64 a_size, u64 b_size,
                                            u64 c_size) {
  // std::string key = "kogge_stone_ppa_add" + std::to_string(0) + "_" +
  //                   std::to_string(a_size) + "x" + std::to_string(b_size) +
  //                   "x" + std::to_string(c_size);
  auto key = hash(__FUNCTION__, a_size, b_size, c_size, 0);
...
KoggeStoneLibrary::BetaCircuit* KoggeStoneLibrary::int_int_add_msb(u64 size) {
  // std::string key = "kogge_stone_ppa_add_msb" + std::to_string(0) + "_" +
  //                   std::to_string(size) + "x" + std::to_string(size) + "x" +
  //                   std::to_string(size);
  auto key = hash(__FUNCTION__, size, size, size, 0);
  // std::unordered_map<std::string, BetaCircuit *>::iterator iter =
  auto iter = mCirMap.find(key);
  if (iter != mCirMap.end())
    return iter->second;
